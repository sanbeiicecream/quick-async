  ...
  chatSpace.on('connection', async socket => {
    ...
    if (roomName) {
      if (global.cuidList?.includes?.(cuid)) {
        ...
        const cuid = uuidv4()
        logger.info(
          `空间已创建-${roomName}，socketID: ${socket.id}, cuid: ${cuid}`
        )
...
function link(io: Server) {
  ...
  linkSpace.on('connection', async socket => {
    try {
      ...
      const joinName =
        socket.handshake.url.match(/[?&]joinName=([^&]*)/)?.[1] || ''
      ...
      if (!chatRoom?.size) {
        ...
        chatSpace
          .to(roomName)
          .to(socketIdOfRoom)
          ?.emit?.('join', { uid, ip, name: joinName })
